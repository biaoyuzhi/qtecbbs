...
layui.define(['laypage', 'fly', 'element', 'flow'], function(exports){
  ...
  if($('#LAY_coop')[0]){
    ...
    $('#LAY_coop .uploadRes').each(function(index, item){
      ...
      upload.render({
        elem: item
        ,url: '/file/upload?filename='+ othis.data('filename')
        ,accept: 'file'
        ,exts: 'zip'
        ,size: 30*1024
        ,before: function(){
  ...
  form.on('submit(reg)', function(data){
      ...
      $.post("/user/register",{
        ...
        layer.msg("注册成功",{icon:16, shade: 0.1, time:1000},
            function(){
  ...
  form.on('submit(set-info)', function (data) {
    ...
    $.post("/user/update",{
      ...
      layer.msg("修改成功",{icon:16, shade: 0.1, time:1000},
          function(){
             location.reload();
      });
    });
